...
#define SYNTHESIZE_SINGLETON_FOR_CLASS_HEADER(__CLASSNAME__)	\
	\
+ (__CLASSNAME__ *)shared##classname;	\
...
{	\
	@synchronized(self){	\
		if(_sharedInstance == nil){	\
			...
			if(_sharedInstance){	\
				method_exchangeImplementations(class_getClassMethod(self, @selector(shared##__CLASSNAME__)), class_getClassMethod(self, @selector(sharedInstanceNoSynch)));	\
				\
