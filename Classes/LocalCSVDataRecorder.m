...
#import "LocalCSVDataRecorder.h"
#import "DataCollector.h"
#include <math.h>

@implementation LocalCSVDataRecorder
...
- (id<DataRecorder>)sharedRecorder {
    if ((self = [super init])) {
        ...
        if (success) {
            ...
            self.readbytes = 0;
            [self.file writeData:[[[self.params componentsJoinedByString:@","] stringByAppendingString:@"\n"] dataUsingEncoding:NSUTF8StringEncoding]];
            self.readbytes = [self.file offsetInFile];
...
- (void)writeDataToDisk:(id)vals {
    ...
    NSString *record = [[vals componentsJoinedByString:@","] stringByAppendingString:@"\n"];
    @synchronized(self) {
        ...
        @catch (NSException * e) {
            ...
            @try { [self applicationWillTerminate]; }
